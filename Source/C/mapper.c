...
unsigned char map_palet1[] = { 126, 247, 247, 109, 247, 120, 108, 116, 247, 106, 113, 118, 107, 117, 119,
  110, 88
};
...
DO_MAP(map_exit) {
  ...
  if (*arg2 == 0) {
    ...
    if (*arg3) {
      set_nest_node(ses->list[LIST_VARIABLE], arg3,
                    "{command}{%s}{destination}{%d}{dir}{%d}{flags}{%d}{name}{%s}{vnum}{%d}",
                    exit->cmd, tunnel_void(ses, ses->map->in_room, exit->vnum, exit->dir), exit->dir, exit->flags, exit->name, exit->vnum);
    } else {
...
DO_MAP(map_goto) {
  ...
  search_keywords(ses, arg, buf, var);

  room = find_room(ses, buf);
...
DO_MAP(map_info) {
  ...
  tintin_printf2(ses, "%+16s %-7s %+16s %-7s %+16s %-7s", "Vtmap:",
                 HAS_BIT(ses->map->flags, MAP_FLAG_VTMAP) ? "on" : "off",
                 "Static:", HAS_BIT(ses->map->flags, MAP_FLAG_STATIC) ? "on" : "off", "Vtgraphics:", HAS_BIT(ses->map->flags, MAP_FLAG_VTGRAPHICS) ? "on" : "off");
  tintin_printf2(ses, "%+16s %-7s %+16s %-7s %+16s %-7s", "Asciigraphics:",
                 HAS_BIT(ses->map->flags,
                         MAP_FLAG_ASCIIGRAPHICS) ? "on" : "off",
                 "Asciivnums:", HAS_BIT(ses->map->flags, MAP_FLAG_ASCIIVNUMS) ? "on" : "off", "Nofollow:", HAS_BIT(ses->map->flags, MAP_FLAG_NOFOLLOW) ? "on" : "off");
  ...
  tintin_printf2(ses, "%+16s %-7s %+16s %-7s %+16s %-7s", "Avoid:",
                 HAS_BIT(ses->map->room_list[ses->map->in_room]->flags,
                         ROOM_FLAG_AVOID) ? "on" : "off", "Hide:",
                 HAS_BIT(ses->map->room_list[ses->map->in_room]->flags, ROOM_FLAG_HIDE) ? "on" : "off", "Leave:", HAS_BIT(ses->map->room_list[ses->map->in_room]->flags, ROOM_FLAG_LEAVE) ? "on" : "off");
  tintin_printf2(ses, "%+16s %-7s %+16s %-7s", "Void:",
                 HAS_BIT(ses->map->room_list[ses->map->in_room]->flags, ROOM_FLAG_VOID) ? "on" : "off", "Static:", HAS_BIT(ses->map->room_list[ses->map->in_room]->flags, ROOM_FLAG_STATIC) ? "on" : "off");
...
DO_MAP(map_legend) {
  ...
  if (*arg1 == 0) {
    if (HAS_BIT(ses->map->flags, MAP_FLAG_VTGRAPHICS)) {
      tintin_printf2(ses,
                     "#MAP: Current legend: \033[12m%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\033[10m",
                     ses->map->legend[0], ses->map->legend[1],
                     ses->map->legend[2], ses->map->legend[3],
                     ses->map->legend[4], ses->map->legend[5],
                     ses->map->legend[6], ses->map->legend[7],
                     ses->map->legend[8], ses->map->legend[9],
                     ses->map->legend[10], ses->map->legend[11], ses->map->legend[12], ses->map->legend[13], ses->map->legend[14], ses->map->legend[15], ses->map->legend[16]);
    } else {
      tintin_printf2(ses,
                     "#MAP: Current legend: %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s",
                     ses->map->legend[0], ses->map->legend[1],
                     ses->map->legend[2], ses->map->legend[3],
                     ses->map->legend[4], ses->map->legend[5],
                     ses->map->legend[6], ses->map->legend[7],
...
DO_MAP(map_list) {
  ...
  for (room = 0; room < ses->map->size; room++) {
    if (match_room(ses, room, buf)) {
      if (*var) {
        add_nest_node(ses->list[LIST_VARIABLE], var, "{%d} {%.3f}",
                      ses->map->room_list[room]->vnum, ses->map->search_stamp == ses->map->room_list[room]->search_stamp ? ses->map->room_list[room]->length : -1);
      } else {
        tintin_printf2(ses, "vnum: %5d   distance: %8.3f   name: %s",
                       ses->map->room_list[room]->vnum, ses->map->search_stamp == ses->map->room_list[room]->search_stamp ? ses->map->room_list[room]->length : -1, ses->map->room_list[room]->name);
      }
    }
  }
}
...
DO_MAP(map_write) {
  ...
  fprintf(file, "L %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n\n",
          ...
          ses->map->legend[9], ses->map->legend[10], ses->map->legend[11], ses->map->legend[12], ses->map->legend[13], ses->map->legend[14], ses->map->legend[15], ses->map->legend[16]);
  ...
  for (vnum = 0; vnum < ses->map->size; vnum++) {
    if (ses->map->room_list[vnum]) {
      fprintf(file,
              ...
              ses->map->room_list[vnum]->area, ses->map->room_list[vnum]->note, ses->map->room_list[vnum]->terrain, ses->map->room_list[vnum]->data, ses->map->room_list[vnum]->weight);

      for (exit = ses->map->room_list[vnum]->f_exit; exit; exit = exit->next) {
        fprintf(file, "E {%5d} {%s} {%s} {%d} {%d} {%s}\n", exit->vnum, exit->name, exit->cmd, exit->dir, exit->flags, exit->data);
      }
    }
  }
...
void create_map(struct session *ses, char *arg) {
  ...
  if (!HAS_BIT(ses->flags, SES_FLAG_UTF8)) {
    ...
    create_legend(ses,
                  "{\xE2\x80\xA2}{\xE2\x95\xB9}{\xE2\x95\xBA}{\xE2\x94\x97}{\xE2\x95\xBB}{\xE2\x94\x83}{\xE2\x94\x8F}{\xE2\x94\xA3}{\xE2\x95\xB8}{\xE2\x94\x9B}{\xE2\x94\x81}{\xE2\x94\xBB}{\xE2\x94\x93}{\xE2\x94\xAB}{\xE2\x94\xB3}{\xE2\x95\x8B}{X}");
  }
}
...
void delete_exit(struct session *ses, int room, struct exit_data *exit) {
  ...
  UNLINK(exit, ses->map->room_list[room]->f_exit, ses->map->room_list[room]->l_exit) free(exit);
}
...
int follow_map(struct session *ses, char *argument) {
  ...
  if (!HAS_BIT(ses->map->flags, MAP_FLAG_STATIC)
      && !HAS_BIT(ses->map->room_list[ses->map->in_room]->flags, ROOM_FLAG_STATIC)) {
    struct listnode *node;
  ...
  while (head != tail) {
    ...
    for (exit = room->f_exit; exit; exit = exit->next) {
      ...
      if (HAS_BIT(exit->flags, EXIT_FLAG_HIDE)
          || HAS_BIT(ses->map->room_list[exit->vnum]->flags, ROOM_FLAG_HIDE)) {
        continue;
      }
    ...
    switch (line) {
      ...
      case 2:
        if (HAS_BIT(room->flags, ROOM_FLAG_VOID)
            || !HAS_BIT(ses->map->flags, MAP_FLAG_ASCIIVNUMS)) {
        ...
        if (room->vnum == ses->map->in_room) {
          ...
          if (HAS_BIT(room->flags, ROOM_FLAG_VOID)) {
            ...
            if (HAS_BIT(ses->map->flags, MAP_FLAG_ASCIIVNUMS)) {
              cat_sprintf(buf, "%s%05d%s", HAS_BIT(room->flags, ROOM_FLAG_PATH)
                          && room->search_stamp == ses->map->search_stamp ? ses->map->path_color : *room->color ? room->color : ses->map->room_color, room->vnum, ses->map->exit_color);
            } else if (strip_color_strlen(ses, room->symbol) <= 1) {
              cat_sprintf(buf, "%s[%-1s%s]%s", HAS_BIT(room->flags, ROOM_FLAG_PATH)
                          && room->search_stamp == ses->map->search_stamp ? ses->map->path_color : *room->color ? room->color : ses->map->room_color, room->symbol, HAS_BIT(room->flags, ROOM_FLAG_PATH)
                          && room->search_stamp == ses->map->search_stamp ? ses->map->path_color : *room->color ? room->color : ses->map->room_color, ses->map->exit_color);
            } else {
              cat_sprintf(buf, "%s%-3s%s", HAS_BIT(room->flags, ROOM_FLAG_PATH)
                          && room->search_stamp == ses->map->search_stamp ? ses->map->path_color : *room->color ? room->color : ses->map->room_color, room->symbol, ses->map->exit_color);
            }
          }
        }
...
int match_room(struct session *ses, int room, char *arg) {
  ...
  if (*arg1) {
    if (!match(ses, ses->map->room_list[room]->name, arg1, SUB_VAR | SUB_FUN)
        && atoi(arg1) != ses->map->room_list[room]->vnum) {
      return 0;
    }
  }
  ...
  while (ses->map->grid_head) {
    ...
    for (exit = room->f_exit; exit; exit = exit->next) {
      if (HAS_BIT(exit->flags, EXIT_FLAG_AVOID)
          || HAS_BIT(ses->map->room_list[exit->vnum]->flags, ROOM_FLAG_AVOID)) {
        continue;
      }
...
void shortest_path(struct session *ses, int run, char *delay, char *arg) {
  ...
  dest = searchgrid_find(ses, ses->map->in_room, buf);

  if (dest == 0) {
    tintin_printf2(ses, "#SHORTEST PATH: NO PATH FOUND TO %s.", buf);
    return;
  }

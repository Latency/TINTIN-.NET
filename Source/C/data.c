  ...
  switch (list_table[root->type].args) {
    case 3:
      tintin_printf2(root->ses,
                     "%*s#%s \033[1;31m{\033[0m%s\033[1;31m}\033[1;36m=\033[1;31m{\033[0m%s\033[1;31m} \033[1;36m@ \033[1;31m{\033[0m%s\033[1;31m}",
...
DO_COMMAND(do_message) {
  ...
  if (*left == 0) {
    ...
    for (index = found = 0; index < LIST_MAX; index++) {
      if (!is_abbrev(left, list_table[index].name_multi)
          && strcasecmp(left, "ALL")) {
        continue;
      }
      if (*right == 0) {

...
DO_CURSOR(cursor_delete) {
  ...
  if (HAS_BIT(gtd->ses->flags, SES_FLAG_BIG5)
      && (gtd->input_buf[gtd->input_cur] & 128) == 128) {
...
DO_CURSOR(cursor_history_next) {
  ...
  if (HAS_BIT(gtd->flags, TINTIN_FLAG_HISTORYSEARCH)) {
    ...
    if (root->update < root->used) {
      input_printf("\033[%dG  \033[0K%.*s\033[%dG",
                   gtd->input_off + inputline_cur_str_len() + 2,
                   gtd->input_off + inputline_max_str_len() - inputline_cur_str_len() - 4, root->list[root->update]->left, gtd->input_off + gtd->input_pos - gtd->input_hid);
    }
    return;
  }
...
DO_CURSOR(cursor_history_find) {
  ...
  if (root->update >= 0) {
    input_printf("\033[%dG ]  %.*s\033[%dG",
                 gtd->input_off + inputline_cur_str_len(),
                 gtd->input_off + inputline_max_str_len() - inputline_cur_str_len() - 4, root->list[root->update]->left, gtd->input_off + gtd->input_pos - gtd->input_hid);
  } else {
...
DO_CURSOR(cursor_left) {
  if (gtd->input_cur > 0) {
    ...
    if (HAS_BIT(gtd->ses->flags, SES_FLAG_BIG5)
        && inputline_str_chk(0, gtd->input_cur) == FALSE) {
      gtd->input_cur--;
      gtd->input_pos--;
...
DO_CURSOR(cursor_redraw_line) {
  ...
  if (gtd->input_hid) {
    if (gtd->input_hid + inputline_max_str_len() >= inputline_raw_str_len(0, gtd->input_len)) {
      input_printf("<%.*s\033[%dG",
                   inputline_str_raw_len(gtd->input_hid + 1,
                                         gtd->input_hid + inputline_max_str_len() - 0), &gtd->input_buf[inputline_str_raw_len(0, gtd->input_hid + 1)], gtd->input_off + gtd->input_pos - gtd->input_hid);
    } else {
      input_printf("<%.*s>\033[%dG",
                   inputline_str_raw_len(gtd->input_hid + 1,
                                         gtd->input_hid + inputline_max_str_len() - 1), &gtd->input_buf[inputline_str_raw_len(0, gtd->input_hid + 1)], gtd->input_off + gtd->input_pos - gtd->input_hid);
    }
  } else {
    if (gtd->input_hid + inputline_max_str_len() >= inputline_raw_str_len(0, gtd->input_len)) {
      input_printf("%.*s\033[%dG",
                   inputline_str_raw_len(gtd->input_hid + 0,
                                         gtd->input_hid + inputline_max_str_len() - 0), &gtd->input_buf[inputline_str_raw_len(0, gtd->input_hid + 0)], gtd->input_off + gtd->input_pos - gtd->input_hid);
    } else {
      input_printf("%.*s>\033[%dG",
                   inputline_str_raw_len(gtd->input_hid + 0,
                                         gtd->input_hid + inputline_max_str_len() - 1), &gtd->input_buf[inputline_str_raw_len(0, gtd->input_hid + 0)], gtd->input_off + gtd->input_pos - gtd->input_hid);
    }
  }
}
...
DO_CURSOR(cursor_test) {
  tintin_printf2(gtd->ses,
                 "bar_len %d off %d pos %d hid %d cur %d len %d max %d tot %d",
                 inputline_max_str_len(), gtd->input_off, gtd->input_pos, gtd->input_hid, gtd->input_cur, gtd->input_len, inputline_max_str_len(), inputline_cur_str_len());
}
...
DO_CURSOR(cursor_mixed_tab_forward) {
  ...
  tab_found = cursor_tab_add(input_now, TRUE)
   || cursor_auto_tab_add(input_now, TRUE);

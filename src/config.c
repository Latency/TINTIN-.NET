...
DO_COMMAND(do_configure) {
  ...
  if (*left == 0) {
    ...
    for (index = 0; *config_table[index].name != 0; index++) {
      ...
      if (node) {
        tintin_printf2(ses, "[%-13s] [%8s] %s", node->left, node->right, strcmp(node->right, "ON") == 0 ? config_table[index].msg_on : config_table[index].msg_off);
      }
    }
...
DO_CONFIG(config_256color) {
  if (!strcasecmp(arg, "AUTO")) {
    if (!strcasecmp(gtd->term, "xterm") || strstr(gtd->term, "256color")
        || strstr(gtd->term, "256COLOR")) {

...
int is_color_code(char *str) {
  if (str[0] == '<') {
    if (!isalnum((int)str[1]) || !isalnum((int)str[2])
        || !isalnum((int)str[3]) || str[4] != '>') {
      return FALSE;
...
void show_debug(struct session *ses, int index, char *format, ...) {
  ...
  if (!HAS_BIT(root->flags, LIST_FLAG_DEBUG)
      && !HAS_BIT(root->flags, LIST_FLAG_LOG)) {
    pop_call();
    return;
  }
...
long long display_timer(struct session *ses, int timer) {
  ...
  tintin_printf2(ses, "%-30s%8lld       %8lld      %8.2f     %8.2f",
                 ...
                 gtd->timer[timer][3] / gtd->timer[timer][4] / 1000, 100.0 * (double)indicated_usage / (double)gtd->total_io_exec, 100.0 * (double)indicated_usage / (double)total_usage);

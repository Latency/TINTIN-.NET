...
void chat_printf(char *format, ...) {
  ...
  for (buddy = gtd->chat->next; buddy; buddy = buddy->next) {
    if (HAS_BIT(buddy->flags, CHAT_FLAG_FORWARD)
        && !HAS_BIT(buddy->flags, CHAT_FLAG_FORWARDALL)) {
...
DO_CHAT(chat_who) {
  ...
  for (buddy = gtd->chat->next; buddy; buddy = buddy->next) {
    tintin_printf(NULL, " %03d %-15s  %s%s%s%s%s  %-20s  %-5u  %-20s",
                  ...
                  HAS_BIT(buddy->flags, CHAT_FLAG_FORWARD) ? "F" : HAS_BIT(buddy->flags, CHAT_FLAG_FORWARDBY) ? "f" : " ", " ", buddy->ip, buddy->port, buddy->version);
  }
  tintin_printf(NULL, "     ===============  =====  ====================  =====  ==================== ");
}
  ...
  *pto++ = CHAT_FILE_BLOCK;
  ...
  if (i < BLOCK_SIZE) {
    chat_printf("File transfer: %s, to %s completed at %lld.%lld KB/s.",
                buddy->file_name, buddy->name, 1000LL * buddy->file_size / (utime() - buddy->file_start_time), 10000LL * buddy->file_size / (utime() - buddy->file_start_time) % 10);
...
void receive_block(unsigned char *s, struct chat_data *buddy, int len) {
  ...
  if (buddy->file_block_cnt == buddy->file_block_tot) {
    ...
    chat_printf
     ("Transfer of %s completed, size: %lld, speed: %lld.%lld KB/s.",
      buddy->file_name, buddy->file_size, 1000LL * buddy->file_size / (utime() - buddy->file_start_time), 10000LL * buddy->file_size / (utime() - buddy->file_start_time) % 10);
...
DO_CHAT(chat_group) {
  ...
  if (*left == 0) {
    ...
    for (buddy = gtd->chat->next; buddy; buddy = buddy->next) {
      tintin_printf(NULL, " %03d %-15s  %-20s  %-5u  %-20s", cnt++, buddy->name, buddy->ip, buddy->port, buddy->group);
    }
    tintin_printf(NULL, "     ===============  ====================  =====  ==================== ");

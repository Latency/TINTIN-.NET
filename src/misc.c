...
DO_COMMAND(do_info) {
  ...
  for (cnt = 0; cnt < LIST_MAX; cnt++) {
    ...
    tintin_printf2(ses, "%-20s  %5d  IGNORE %3s  MESSAGE %3s  DEBUG %3s",
                   ...
                   ses->list[cnt]->used,
                   HAS_BIT(ses->list[cnt]->flags,
                           LIST_FLAG_IGNORE) ? "ON" : "OFF", HAS_BIT(ses->list[cnt]->flags, LIST_FLAG_MESSAGE) ? "ON" : "OFF", HAS_BIT(ses->list[cnt]->flags, LIST_FLAG_DEBUG) ? "ON" : "OFF");
  }
  tintin_header(ses, "");
